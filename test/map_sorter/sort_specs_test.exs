defmodule MapSorter.SortSpecsTest do
  ...
  describe "SortSpecs.to_comp_fun/1" do
    ...
    test "returns a sort function", %{setup: setup} do
      Logger.configure(level: @level)
      comp_fun = SortSpecs.to_comp_fun([:dob, desc: :likes])
      Logger.configure(level: :error)
      assert comp_fun == setup.here_fun
      assert is_function(comp_fun, 2)
    ...
    test ~S[returns a "true" function given bad specs], %{setup: setup} do
      Logger.configure(level: @level)
      tuple_comp_fun = SortSpecs.to_comp_fun({:dob, :desc, :likes})
      empty_comp_fun = SortSpecs.to_comp_fun([])
      nihil_comp_fun = SortSpecs.to_comp_fun(nil)
      Logger.configure(level: :error)
